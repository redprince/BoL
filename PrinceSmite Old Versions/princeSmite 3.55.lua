...
local smiteDamage = 
    function(target)
        if smiteSkill then
            if smiteSkill == SUMMONER_1 then
                smiteTMP = 20
            else
                smiteTMP = 21
            end
            
            if myHero:CanUseSpell(smiteTMP) or myHero:CanUseSpell(smiteSkill) then 
...
function princeCastSmite(target)
    if not casted_one then
        if VIP_USER and PrinceSmite.packetCast and 1 == 2 then
