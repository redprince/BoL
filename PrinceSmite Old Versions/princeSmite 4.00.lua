...
PrinceSmite:addSubMenu("AutoCast Spells", "acs")
    ...
    PrinceSmite.acs:addParam("JarvanIVQ", "JarvanIV Q", SCRIPT_PARAM_ONOFF, true)
...
function OnTick()
    ...
    if PrinceSmite.on then
        ...
        if string.match(myHero:GetSpellData(smiteSkill).name, "player") then
...
function checkAutoCast()
    ...
    elseif myHero.charName == "Chogath" and PrinceSmite.acs.choR then
        return true
    elseif myHero.charName == "JarvanIV" and PrinceSmite.acs.JarvanIVQ then
        return true
...
function princeCastSmite(target)
    if not casted_one then
        if VIP_USER and PrinceSmite.packetCast then
