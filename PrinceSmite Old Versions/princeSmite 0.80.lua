...
if myHero.charName == "Nunu" then
    spellSlot = _Q
    spellDamage = function(target) return 250 + (150 * myHero:GetSpellData(_Q).level) end
    spellRange = 175
...
elseif myHero.charName == "Elise" then
    spellSlot = _Q
    spellDamage = function(target) return 5 + (35 * myHero:GetSpellData(_Q).level) + (target.health * (0.08 + (0.03 * math.floor(myHero.ap / 100)))) end
    spellRange = 475
end
...
function OnTick()
    ...
    if PrinceSmite.on and insideJungle then
        ...
        for i, mob in pairs(jungleMobs.objects) do
            ...
            if not mob.dead and PrinceSmite.mobs[mob.charName:gsub("_", "")] then
                -- check for spells
                if spellDamage(mob) > 0 
                and GetDistance(mob) < spellRange 
                and mob.health <= smiteDamage + spellDamage(mob) 
                and myHero:GetSpellData(spellSlot).currentCd < 0.01
                then
                    CastSpell(spellSlot, mob)
                -- check for smite
                elseif GetDistance(mob) < SMITE_RANGE 
                and mob.health <= smiteDamage 
                and myHero:CanUseSpell(smiteSkill) then
    ...
    if PrinceSmite.on and checkInJungle < os.clock() then
        ...
        if insideJungle then
            ...
            if     myHero.level > 13 then smiteDamage = 100 + (myHero.level * 50)
            elseif myHero.level >  8 then smiteDamage = 240 + (myHero.level * 40)
            elseif myHero.level >  3 then smiteDamage = 330 + (myHero.level * 30)
            elseif myHero.level >  1 then smiteDamage = 370 + (myHero.level * 20)
            end
        end
    ...
    if PrinceSmite.hpbar.draw then
        for i, mob in pairs(jungleMobs.objects) do
            ...
            then
                ...
                local smiteDamageDistance = (smiteDamage / mob.maxHealth) * distance
                local comboDamage = smiteDamage + spellDamage(mob)
